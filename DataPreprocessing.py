...
import string
import pandas as pd
from DataCleaningTechniques import *
...
allowedWordTypes = ["J", "R", "V",
                    "N"]  # J is Adject, R is Adverb, V is Verb, N is Noun. These are used for POS Tagging
...
def tokenize(text, wordCountBefore):
    ...
    tokens = replaceNegations(
        tokens)  # Technique 6: finds "not" and antonym for the next word and if found, replaces not and the next word with the antonym
    ...
    with open("result.txt", "a") as result:
        result.write(onlyOneSentence.lower() + "\n")
...
for line in f['text']:
    ...
    # textID = (columns[0])
    # y = (columns[2])
    text = removeUnicode(line)  # Technique 0
    ...
    text = replaceMultiExclamationMark(
        text)  # Technique 5: replaces repetitions of exlamation marks with the tag "multiExclamation"
